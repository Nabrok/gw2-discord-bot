...
import { Alert, Row, Col, Form, FormGroup, ControlLabel, FormControl, Radio, Panel, Popover, OverlayTrigger, Tabs, Tab, Nav, NavItem } from 'react-bootstrap';
...
class Privacy extends React.Component {
	constructor(props) {
		...
		this.state = {
			privacy: props.privacy || 1
		};
	}

	componentWillReceiveProps(newProps) {
		if (newProps.privacy !== this.props.privacy) this.setState({ privacy: newProps.privacy });
	}
...
class Character extends React.Component {
	constructor(props) {
		...
		this.state = {
			tab: 1,
			type: 'pve',
			character: this._getCharacter(props.name),
			profession: {}
		};
	}

	componentWillReceiveProps(newProps) {
		if (newProps.name !== this.props.name) this._getCharacter(newProps.name);
	}
	...
	_getCharacter(name) {
		Socket.send('get character', name).then(character => {
		...
		return (
			...
			<Row>
				...
				<Col xs={11}>
					...
					<p>Level { this.state.character.level } { this.state.character.profession }</p>
				</Col>
			</Row>
			<Row>
			...
			<Row>
				<Tabs activeKey={this.state.tab} onSelect={this.selectTab.bind(this)} id="character-tabs">
					...
					<Tab title="Privacy" eventKey={4}><Privacy character={this.state.character} privacy={this.props.privacy} /></Tab>
				</Tabs>
			</Row>
			</div>
		);
...
export default class Characters extends React.Component {
	...
	_getPrivacy(character) {
		if (! character) return;
		return PrivacyStore.getPrivacy(character) || 1;
	}
	...
	_onPrivacyChange() {
		if (this.state.selectedCharacter) {
			var selectedPrivacy = this._getPrivacy(this.state.selectedCharacter);
			this.setState({ selectedPrivacy });
		}
	}

	_getCharacters() {
		if (this.state.token.permissions && this.state.token.permissions.indexOf('characters') === -1) return this.setState({ characters: [] });
	...
	_selectCharacter(e) {
		var selectedCharacter = e.target.value;
		var selectedPrivacy = this._getPrivacy(selectedCharacter);
		this.setState({ selectedPrivacy, selectedCharacter });
	}
	...
	render() {
		return(<div>
				...
				<Col xs={12}>
					<Form inline>
						<FormGroup controlId="characterSelect">
							...
							{' '}
							<FormControl componentClass="select" placeholder="Character" value={this.state.selectedCharacter} onChange={this._selectCharacter}>
								<option value=""></option>
			...
			{ this.state.selectedCharacter && <Character name={this.state.selectedCharacter} privacy={this.state.selectedPrivacy} /> }
		</div>);
	}
}
