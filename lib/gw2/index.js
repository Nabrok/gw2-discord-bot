...
api.prototype.addHook = function(path, hook) {
		...
		.then(cache => {
			...
			return new Promise((resolve, reject) => {
				...
				request(options, function(error, http_res, result) {
					...
					if (! http_res.statusCode.toString().match(/^[2|4]/)) reject(new Error(http_res.statusCode));
					if (result.text) reject(new Error(result.text));
...
api.prototype.keepUpdated = function(path, key, frequency) {
	...
	var thing_to_do = function() {
		self.request(path, key, function() {
	...
	if (run_immediately) this.request(path, key, null, options);
};
...
api.prototype.useCache = function(path, ids, cache_name, callback) {
	...
	var promise = this.checkBuild()
		.then(changed => new Promise((resolve, reject) => {
			// If changed give a short time out to make sure database has finished clearing
			if (changed) setTimeout(resolve, 500);
			else resolve();
		}))
...
api.prototype.checkBuild = function(callback, clear) {
	...
	var promise = Promise.all([
		...
		if (! db_build || parseInt(db_build.id) !== parseInt(gw_build.id) || clear) {
			return db.setObjectAsync('gw2:build', gw_build).then(() => {
