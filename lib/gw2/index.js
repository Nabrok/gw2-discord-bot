...
api.prototype.request = function(path, key, callback, options) {
	...
	var promise = db.getCachedResponse(path, key)
		.then(cache => {
			if (cache) {
			...
			return new Promise((resolve, reject) => {
				...
				return db.saveCachedResponse(path, key, result, ttl)
					.then(() => {
...
api.prototype.keepUpdated = function(path, key, frequency) {
	...
	if (timers[timer_key]) {
		clearTimeout(timers[timer_key].timeout); // clear previous timeout if there is one
...
api.prototype.clearCache = function(cache_name, callback) {
	var promise = Promise.all(
		[db.clearCache(cache_name)]
			.concat(languages.map(l => db.clearCache(l+':'+cache_name)))
	);
	if (callback) {
...
api.prototype.useCache = function(path, ids, cache_name, callback) {
	...
	var promise = this.checkBuild()
		.then(() => db.getCacheKeys(cache_name))
		.then(db_ids => {
			var to_get = [];
			var in_db = [];
			ids.forEach(i => {
			...
			if (in_db.length > 0) {
				retrieve.push(db.getCache(cache_name, in_db));
			} else {
				retrieve.push([]);
			}
			return Promise.all(retrieve).then(result => {
				...
				return Promise.all(api_res.filter(item => !!item).map(item => {
					...
					return db.setCache(cache_name, item.id, item);
				})).then(() => return_items);
			});
		})
	;
