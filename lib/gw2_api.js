...
api.setLanguage = function(lang) {
	if (! lang.match(/^(en|es|de|fr|ko|zh)$/)) return;
	api.language = lang;
};
...
api.clearCache = function(cache_name, callback) {
	db.clearCache(cache_name, callback);
};
	...
	async.series([
		...
		if (to_get.length > 0) {
			...
			while (to_get.length > 0) {
				...
				requests.push(async.apply(api.request, path+'?ids='+this_bit.join(',')+'&lang='+api.language, null));
			}
			retrieve.api = function(next) {
				async.parallel(requests, function(err, result) {
					...
					result.forEach(function(r) {
						all_results = all_results.concat(r);
					});
